...
class XL_LUU_TRU {
  ...
  Phuc_hoi_Doi_tuong_da_Xoa(Loai_Doi_tuong, Doi_tuong) {
    var Kq = ""
    try {

      var Duong_dan = Thu_muc_Du_lieu + "//" + Loai_Doi_tuong + "//" + Doi_tuong.Ma_so + "." + Cong_nghe
      var Duong_dan_da_Xoa = Thu_muc_Du_lieu + "//" + Loai_Doi_tuong + "//Xoa//" + Doi_tuong.Ma_so + "." + Cong_nghe
      File.unlinkSync(Duong_dan_da_Xoa)
      var Chuoi = JSON.stringify(Doi_tuong,null,"\t")
      File.writeFileSync(Duong_dan, Chuoi, "UTF-8")
    } catch (Loi) {
      Kq = Loi
